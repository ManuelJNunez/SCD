...
int producir(int id)
{
   static int contador[num_productores] = {0};
   sleep_for( milliseconds( aleatorio<10,100>()) );
   int producido = id*(num_items/num_productores)+contador[id];
   contador[id]++;
...
void funcion_buffer()
{
   ...
   for( unsigned int i=0 ; i < num_items*2; i++ ){
      ...
      if(estado.MPI_TAG == etiq_prod){ // si ha sido el productor: insertar en buffer
            buffer[primera_libre] = valor;
            primera_libre = (primera_libre+1) % tam_vector;
            num_celdas_ocupadas++;
            cout << "Buffer ha recibido valor " << valor << endl;
      }
      else if(estado.MPI_TAG == etiq_cons){ // si ha sido el consumidor: extraer y enviarle
